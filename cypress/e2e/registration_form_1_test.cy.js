...
// I tested Our first test on 14/06/2023.
describe('This is first test suite,Dhruv Deveshkumar Shah', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Dhruv')
    ...
    */
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // Assert that username has tooltip with error message
        cy.get('input[name="password"]').should('have.attr', 'placeholder').should('contain', 'Qwerty')
    ...
    it('User cannot add letter to the phone number', () => {
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').type('sfhsfsjfsdfs')
        cy.get('input[name="password"]').type('MyPass')
        cy.get('[name="confirm"]').type('MyPass')

        // How we check that from HTML code phone number only contain numbers.
